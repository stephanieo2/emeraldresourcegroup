...
class Custom_Widget_Text extends WP_Widget {
	...
	function __construct() {
        $widget_ops = array(
            ...
            'description' => __('Arbitrary text or HTML', 'emeraldresourcegroup')
        );
        $control_ops = array(
            'width' => 400,
            'height' => 350
        );
        //$this->WP_Widget('text', __('Text', 'emeraldresourcegroup'), $widget_ops, $control_ops);
        parent::__construct('text', __('Text', 'emeraldresourcegroup'), $widget_ops, $control_ops);
	}
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'emeraldresourcegroup'); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<textarea class="widefat" rows="16" cols="20" id="<?php echo $this->get_field_id('text'); ?>" name="<?php echo $this->get_field_name('text'); ?>"><?php echo $text; ?></textarea>

		<p><input id="<?php echo $this->get_field_id('filter'); ?>" name="<?php echo $this->get_field_name('filter'); ?>" type="checkbox" <?php checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php _e('Automatically add paragraphs', 'emeraldresourcegroup'); ?></label></p>
<?php
	}
}
...
class Widget_Recent_Posts_From_Category extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'widget_recent_entries_from_category', 'description' => __( 'The most recent posts from specific category on your site', 'emeraldresourcegroup') );
        parent::__construct('recent-posts-from-category', __('Recent Posts From Specific Category', 'emeraldresourcegroup'), $widget_ops);
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? __('Recent Posts', 'emeraldresourcegroup') : $instance['title'], $instance, $this->id_base);
...
<p>
<label>
<?php _e( 'Category', 'emeraldresourcegroup' ); ?>:
<?php wp_dropdown_categories( array( 'name' => $this->get_field_name("cat"), 'selected' => $instance["cat"] ) ); ?>
</label>
</p>

<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of posts to show:', 'emeraldresourcegroup'); ?></label>
<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
<?php
    }
}

function custom_recent_post() {
...
add_action('widgets_init', 'custom_recent_post');
